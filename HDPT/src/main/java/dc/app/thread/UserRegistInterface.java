 ...
 */
public class UserRegistInterface extends AbstractThread{
    private static final Logger LOGGER = LoggerFactory.getLogger(UserRegistInterface.class);
     ...
     * @param account
     */
    public UserRegistInterface(YyUserAccount account){
    ...
    protected void recvResponse(Map<String, Object> params) {
        ...
        if (!"0".equals(errorCcode)) {
            ...
            LOGGER.error("Receive error when request registInterface!-->errorCode[{}] errorMsg[{}]", errorCcode, errorMsg);
        }else{
            LOGGER.debug("Request registInterface success!");
        }
    }
    ...
    public void run() {
        if(account == null){
            LOGGER.error("YyUserAccount is null!Can not request registInterface!");
            return ;
        }
