...
public class TwdChartService 
{
 ...
 private void initTopChart(ChartProcessor chart,Map map)
 {
  ...
  chart.addBackgroundValue("病案号：", x_length-x_unit-2, bottom_length+y_grid_length+top_row_height*5 + 0.1, font);
  ...
  chart.addBackgroundValue("杭州市一医院", x_length/2, bottom_length+y_grid_length+top_row_height*8 + 0.1, new Font("SansSerif", Font.PLAIN, 20));
...
//  List<String> twdDateList=(List<String>)map.get("twdDateList");
  ...
  if(dateList!=null&&dateList.size()>0)
  {
   ...
   {
    chart.addBackgroundValue(dateList.get(i),grid_left*(i+1)+ x_unit / 2,bottom_length+y_grid_length+top_row_height*2+ 0.1, font); //日期
   }
  }
...
//   }
 ...
 public void initBottomChart(ChartProcessor chart,Map map)
 {
  ...
  chart.addBackgroundValue(" 空腹血糖值   mmol/L", grid_left / 2, bottom_length- bottom_row_height * 4 + 0.1, font);
  ...
  chart.addBackgroundValue(" 餐后血糖值   mmol/L", grid_left / 2, bottom_length- bottom_row_height * 5 + 0.1, font);
  ...
  for(int i=0;i<dateList.size();i++)
  {
   PTempInfo resultInfo=tempInfoList.get(i);
    if(resultInfo!=null)
     ...
     chart.addBackgroundValue(resultInfo.getKFXT()==null?"":resultInfo.getKFXT()+"",grid_left*(i+1)+ x_unit / 2,bottom_length- bottom_row_height * 4 + 0.1, font); //空腹血糖
     chart.addBackgroundValue(resultInfo.getCHXT()==null?"":resultInfo.getCHXT()+"",grid_left*(i+1)+ x_unit / 2,bottom_length- bottom_row_height * 5 + 0.1, font); //餐后血糖
     ...
     chart.addBackgroundValue(resultInfo.getWeight(),grid_left*(i+1)+ x_unit / 2,bottom_length- bottom_row_height * 8 + 0.1, font); //体重
     chart.addBackgroundValue(resultInfo.getHeight(),grid_left*(i+1)+ x_unit / 2,bottom_length- bottom_row_height * 9 + 0.1, font); //身高
     }
  }
  }
   ...
   if(resultInfo!=null)
    {
     ...
     for(int j=0;j<detailInfoList.size();j++)  //遍历每天的数据
     {
      ...
      if(null == resultDetailInfo)
      {
        ...
        * 呼吸  
        */
       double y_breathe_height=bottom_length- bottom_row_height+((k%2==0)?-bottom_row_height+0.1:0.1); //如每日记录呼吸2次以上，应当在相应的栏目内上下交错记录
...
//       }
       ...
       if(Math.abs(grid_temperature_height- grid_pulse_height)<0.05 )  //体温 脉搏重合
       {
