...
// Tree object
function dTree(objName, imgRoot) {
    this.config = {
        target                : null,
        folderLinks            : true,
        useSelection        : true,
...
dTree.prototype.node = function(node, nodeId) {
    ...
    if (this.config.useIcons) {
        ...
        if (node.pid != -1 && this.showchkboxflag == 1) {
            ...
            str += '<input type="checkbox" name="ckx' + nodeId + '" value="' + node.id + ',' + node.name + '"';
            if (this.dbchkflag == 1)
                str += ' ondblclick="javascript: ' + this.obj + '.dbck(' + nodeId + ');"';
    ...
    if (node.url) {
        ...
        if (this.isMenu) {
            if(node.id!=null && node.id.indexOf('e_')==-1){
            	str += 'oncontextmenu=' + this.obj + '.Do_right(event,"' + node.id + '",' + nodeId + ') ';
            }
        }
        str += '>';
...
//check all children
dTree.prototype.dbck = function(id) {
    ...
    if (this.checksubflag == 1) {
        for (var n = node._fcp; n <= node._lcp; n++) {
            if (this.aNodes[n] != null && this.aNodes[n].pid == node.id) {
                ...
                this.dbck(this.aNodes[n].nodeId);
            }
        }
    }
};
...
;
dTree.prototype.Do_right = function(event, nodeid, n)
{
    this.nid = nodeid;
    this.s(n);
    OnMouseUp(event);
